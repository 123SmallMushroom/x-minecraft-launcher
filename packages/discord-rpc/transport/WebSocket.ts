...
export class WebSocketTransport extends Transport {
  ...
  connect(): Promise<void> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (!this.ws) { reject(new RPCError(CUSTOM_RPC_ERROR_CODE.RPC_COULD_NOT_CONNECT, 'Failed to connect to websocket')) }
  ...
  close(): Promise<void> {
    if (!this.ws) return new Promise((resolve) => resolve())
